...
The server provides cluster info, API resource discovery, namespaces, nodes, pods, workloads, `kubernetes_resource_search`, `kubernetes_pod_stat`, `kubernetes_node_stat`, services, ConfigMaps, ingresses, EndpointSlices, events, bounded pod logs, bounded pod file reads, allowlisted resource get, and an API health check. `kubernetes_resource_search` accepts a case-insensitive `resource` type (or `workload` for Deployment, StatefulSet, DaemonSet, Job, and CronJob), a name `keyword` expression, an optional namespace, label filters, and limit. In keyword expressions, `&` is AND, `|` is OR, and `*`/`?` are wildcards; matching is case-insensitive and performed against resource names. The two `*_stat` tools read current CPU, memory, and other reported resource usage from `metrics.k8s.io/v1beta1`; they require a working metrics-server or another Metrics API implementation and the caller's RBAC permission to read it. `kubernetes_pod_stat` accepts an optional namespace and pod name and returns per-container plus aggregated usage; `kubernetes_node_stat` accepts an optional node name and returns node usage. Lists accept a flat `filters` string such as `app:payments,environment:prod`; the tool converts it to a Kubernetes label selector. Lists default to a bounded page and cap `limit` at 500. Logs default to 100 lines, are never followed, and are capped at 256 KiB.

`kubernetes_pod_file` reads one absolute regular file from a selected Pod/container through a fixed `/bin/cat -- <path>` exec request; it accepts no arbitrary command and returns at most 1 MiB with `truncated=true` when capped. The caller still needs the Kubernetes `pods/exec` RBAC permission.

The resource allowlist includes namespaces, nodes, pods, ConfigMaps, services, events, deployments, statefulsets, daemonsets, jobs, cronjobs, ingresses, and endpoint slices. Secrets, RBAC objects, arbitrary API paths, watches, arbitrary exec commands, and all write operations are unavailable.
