<script lang="ts">
  ...
  const tools = ['minio_health', 'minio_buckets', 'minio_bucket_objects', 'minio_object_stat', 'minio_bucket_versioning', 'minio_bucket_lifecycle'];
...
<div class="provider-resource-details postgres-resource-details">
  <div class="provider-resource-meta">
    <div><span>接入方式</span><strong class:agent>{agent ? 'Agent · MCP 代理' : 'Direct · S3 API'}</strong></div>
  ...
  <div class="provider-resource-models mcp-resource-tools">
    ...
    {#each tools as tool}<div class="provider-resource-model-row mcp-resource-tool-row"><div><strong>{tool}</strong></div></div>{/each}
  </div>
</div>
