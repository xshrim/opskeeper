...
{:else}
<div class="docker-form-grid"><label class:invalid={configurationAttempted&&!host.trim()}><span><i>*</i>Redis 主机</span><input bind:value={host} required on:input={onConfigurationChange} placeholder="例如 redis.example.com" /></label><label><span>端口</span><input type="number" min="1" max="65535" bind:value={port} on:input={onConfigurationChange}/></label><label><span>数据库编号</span><input type="number" min="0" bind:value={database} on:input={onConfigurationChange}/></label><label><span>用户名（可选）</span><input bind:value={username} on:input={onConfigurationChange}/></label><label><span>密码（可选）</span><PasswordInput bind:value={password} on:input={onConfigurationChange} ariaLabel="密码" /></label><label><span>超时时间（秒）</span><input type="number" min="1" max="300" bind:value={timeoutSeconds} on:input={onConfigurationChange}/></label></div>
{/if}
