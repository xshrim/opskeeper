...
func TestPolicyGatewayAuditsProviderModelTimingAndErrors(t *testing.T) {
	...
	if _, err := gateway.Invoke(context.Background(), ToolCall{ExecutionID: "exec-1", Sequence: 2, ScopeID: "scope-1", ResourceID: "resource-1", Name: "read", ProviderID: "provider-1", ModelName: "model-a", Arguments: map[string]any{"api-key": "secret"}}); err != nil {
		t.Fatal(err)
	}
	...
	if record.ProviderID != "provider-1" || record.ModelName != "model-a" || record.DurationMS < 0 || record.StartedAt.IsZero() || record.CompletedAt.IsZero() {
