...
func (s *PostgresStore) AppendEvent(ctx context.Context, event Event) error {
	if s == nil || s.pool == nil || strings.TrimSpace(event.ExecutionID) == "" {
		return fmt.Errorf("Engine event store is unavailable")
	}
	payload, err := json.Marshal(event.Payload)
	if err != nil {
		return fmt.Errorf("encode Engine event payload: %w", err)
	}
	_, err = s.pool.Exec(ctx, `
		INSERT INTO engine_execution_events (execution_id, sequence, type, status, occurred_at, payload)
		VALUES ($1, $2, $3, $4, $5, $6)
...
func (s *PostgresStore) ListEvents(ctx context.Context, executionID string, after int64, limit int) ([]Event, error) {
	if s == nil || s.pool == nil {
		return nil, fmt.Errorf("Engine event store is unavailable")
	}
	if limit < 1 || limit > 500 {
		limit = 200
	}
	rows, err := s.pool.Query(ctx, `SELECT execution_id, sequence, type, status, occurred_at, payload FROM engine_execution_events WHERE execution_id = $1 AND sequence > $2 ORDER BY sequence LIMIT $3`, executionID, after, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *PostgresStore) RecordToolCall(ctx context.Context, record ToolCallRecord) error {
	if s == nil || s.pool == nil {
		return fmt.Errorf("Engine tool call store is unavailable")
	}
	...
	_, err = s.pool.Exec(ctx, `
		INSERT INTO engine_execution_tool_calls (execution_id, sequence, provider_id, model_name, resource_id, tool_name, arguments, output, status, error_code, error_message, started_at, completed_at, duration_ms)
		...
		ON CONFLICT (execution_id, sequence) DO UPDATE SET provider_id = EXCLUDED.provider_id, model_name = EXCLUDED.model_name, resource_id = EXCLUDED.resource_id, arguments = EXCLUDED.arguments, output = EXCLUDED.output, status = EXCLUDED.status, error_code = EXCLUDED.error_code, error_message = EXCLUDED.error_message, started_at = EXCLUDED.started_at, completed_at = EXCLUDED.completed_at, duration_ms = EXCLUDED.duration_ms`,
		record.ExecutionID, record.Sequence, record.ProviderID, record.ModelName, record.ResourceID, record.ToolName, args, output, record.Status, record.ErrorCode, record.Error, nullableTime(record.StartedAt), nullableTime(record.CompletedAt), record.DurationMS)
	return err
}

func (s *PostgresStore) ListToolCalls(ctx context.Context, executionID string, limit int) ([]ToolCallRecord, error) {
	if s == nil || s.pool == nil {
		return nil, fmt.Errorf("Engine tool call store is unavailable")
	}
	if limit < 1 || limit > 500 {
		limit = 200
	}
	rows, err := s.pool.Query(ctx, `SELECT execution_id, sequence, COALESCE(provider_id::text, ''), COALESCE(model_name, ''), COALESCE(resource_id::text, ''), tool_name, arguments, output, status, COALESCE(error_code, ''), COALESCE(error_message, ''), started_at, completed_at, duration_ms FROM engine_execution_tool_calls WHERE execution_id = $1 ORDER BY sequence LIMIT $2`, executionID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ExecutionID, &item.Sequence, &item.ProviderID, &item.ModelName, &item.ResourceID, &item.ToolName, &arguments, &output, &item.Status, &item.ErrorCode, &item.Error, &item.StartedAt, &item.CompletedAt, &item.DurationMS); err != nil {
			return nil, err
		}
