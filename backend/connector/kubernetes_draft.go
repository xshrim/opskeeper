...
func (s *Service) TestKubernetesDraft(ctx context.Context, input KubernetesDraftInput) (KubernetesDraftCheck, error) {
	...
	connection := kclient.ConnectionInput{Server: input.Server, CA: input.CA, Token: input.Token, ClientCert: input.ClientCert, ClientKey: input.ClientKey, Context: input.Context, SkipTLSVerify: input.SkipTLSVerify}
	if input.Kubeconfig != "" {
