...
8. T10 实施 ADK Agent、Runner、AIModel（内部大模型节点 LLMEndpoint）和 Skill；历史 LLMProvider 仅保留兼容解析。T14 才引入 MCP SDK 并实施 MCP 调用。不得为了登记 MCPServer 资源而在 T10 提前加入未使用的 MCP 运行时依赖。
