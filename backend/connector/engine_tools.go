...
// EngineProvider exposes existing connector capabilities through the
...
func (p connectorContextProvider) Resolve(ctx context.Context, resource engine.ContextResource) ([]engine.Tool, []engine.ContextFact, error) {
	...
	tools := make([]engine.Tool, 0, 4)
	add := func(name, description string, schema json.RawMessage, fn func(context.Context, map[string]any) (engine.ToolResult, error)) {
		tools = append(tools, engine.ToolFunc{Def: engine.ToolDefinition{Name: name, Description: description, InputSchema: schema, Source: "connector", ResourceID: resource.ID, ReadOnly: true}, Fn: fn})
	}
	...
	case "Prometheus":
		add("connector.query_metrics", "Query bounded Prometheus metrics.", metricsSchema, func(runCtx context.Context, args map[string]any) (engine.ToolResult, error) {
		...
		add("connector.get_alerts", "Read active Prometheus alerts.", alertsSchema, func(runCtx context.Context, args map[string]any) (engine.ToolResult, error) {
			return evidenceResult(p.service.GetAlerts(runCtx, resource.ID, AlertsQuery{ActiveOnly: boolArg(args, "active_only", true)}))
		})
	case "Loki":
		add("connector.query_logs", "Query bounded logs.", logsSchema, func(runCtx context.Context, args map[string]any) (engine.ToolResult, error) {
	...
	facts := make([]engine.ContextFact, 0, 1)
	if resource.Kind == "Kafka" {
		...
		facts = append(facts, engine.ContextFact{ResourceID: resource.ID, Kind: resource.Kind, Data: result.Output, Untrusted: true})
	}
...
func evidenceResult(evidence Evidence, err error) (engine.ToolResult, error) {
	if err != nil {
		return engine.ToolResult{}, err
	}
	return engine.ToolResult{Output: evidence, Untrusted: true}, nil
}
