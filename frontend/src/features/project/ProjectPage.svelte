<script lang="ts">
  ...
  async function createProject() {
    ...
    try { const created = await api.createProject(projectTeamId, { name: projectName, code: projectCode, icon: projectIcon, labels: {} }); onProjectCreated(created); onSelectProject(created); projectName = ''; projectCode = ''; onNotice(`项目“${created.name}”已创建`); }
    catch (error) { onError(describeError(error, '创建项目失败')); } finally { creatingProject = false; }
  }
  async function createApplication() {
    if (!selectedProject) return;
    creatingApplication = true; onError('');
    try { await api.createApplication(selectedProject.id, { name: applicationName, code: applicationCode, description: applicationDescription, icon: 'AppWindow', labels: {} }); showApplicationForm = false; applicationName = ''; applicationCode = ''; applicationDescription = ''; await loadWorkspace(selectedProject.id); onNotice('应用已添加'); }
  ...
  function beginEdit(app: Application) { editingAppId = app.id; editName = app.name; editDescription = app.description; }
  async function saveEdit(app: Application) { try { await api.updateApplication(app.id, { name: editName, description: editDescription }); editingAppId = ''; await loadWorkspace(selectedProjectId); onNotice('应用已更新'); } catch (error) { onError(describeError(error, '更新应用失败')); } }
  ...
  async function saveRelation() {
    ...
    try {
      ...
      if (relationMode === 'instance') await api.createApplicationInstance(relationAppId, { name: relationName || 'default', runtime_kind: relationRuntime, target_resource_id: relationResourceId, selector: binding });
      else await api.createApplicationDependency(relationAppId, { target_resource_id: relationResourceId, dependency_kind: relationKind, binding, required: true });
  ...
  <aside class="project-nav panel">
    ...
    <div class="table-list team-list">
      {#each teams as team}
        ...
        <button class:selected={selectedScopeId === team.scope.id} class="list-row" on:click={() => onSelectTeam(team)}><span class="entity-summary"><span class="entity-icon team-icon"><svelte:component this={TeamIcon} size={16} /></span><span><strong>{team.name}</strong><small>{team.code}</small></span></span><span class="row-arrow">›</span></button>
      {/each}
    </div>
    <div class="project-list">
      {#each visibleProjects as project}
        <button class:selected={selectedProjectId === project.id} class="project-row" on:click={() => onSelectProject(project)}><span class="entity-icon project-icon">{iconGlyph(project.icon)}</span><span><strong>{project.name}</strong><small>{project.code}</small></span><span class="status-dot {project.status}"></span></button>
