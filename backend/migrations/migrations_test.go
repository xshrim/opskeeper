...
func TestLoadOrdersEmbeddedMigrations(t *testing.T) {
	...
	if len(items) != 38 {
		t.Fatalf("load() returned %d migrations, want thirty-eight migrations", len(items))
	}
	...
	if items[26].version != 27 || items[26].name != "knowledge_workflow" {
		t.Fatalf("loaded migration = %#v, want version 27 knowledge_workflow", items[26])
	}
	if items[27].version != 28 || items[27].name != "remove_skill_execution_store" {
		t.Fatalf("loaded migration = %#v, want version 28 remove_skill_execution_store", items[27])
	}
	if items[28].version != 29 || items[28].name != "inspection_agent_profile" {
		t.Fatalf("loaded migration = %#v, want version 29 inspection_agent_profile", items[28])
	}
	if items[29].version != 30 || items[29].name != "ai_provider_general_role" {
		t.Fatalf("loaded migration = %#v, want version 30 ai_provider_general_role", items[29])
	}
	if items[30].version != 31 || items[30].name != "mcp_server_transport_and_config" {
		t.Fatalf("loaded migration = %#v, want version 31 mcp_server_transport_and_config", items[30])
	}
	...
	if items[33].version != 34 || items[33].name != "resource_access_mode" {
		t.Fatalf("loaded migration = %#v, want version 34 resource_access_mode", items[33])
	}
	if items[34].version != 35 || items[34].name != "unified_resource_access" { t.Fatalf("loaded migration = %#v, want version 35 unified_resource_access", items[34]) }
