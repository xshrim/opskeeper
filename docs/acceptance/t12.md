...
| 7 | 运行 Kafka 行为测试 | 覆盖 Broker、Topic、分区、ISR、离线副本及 TLS/SASL 配置；能力缺失时可理解降级 | 通过；结构化夹具验证 ISR、离线副本和认证配置；消费组/积压遇到 ACL、版本或协议限制时写入 `unavailable`。 |
...
本阶段不包含自动修复、自动重启、扩缩容、写 SQL、参数修改或任意命令执行。
