<script lang="ts">
  ...
  function select(categoryName: string, subtypeName = '全部') {
    expandedCategory = categoryName === '全部' ? '' : categoryName;
...
<section class="panel resource-list-panel">
  <div class="resource-catalog-rail">
    <button class:active={category === '全部'} class="catalog-root" type="button" on:click={() => select('全部')}>
      <span class="catalog-icon"><ResourceBrandIcon resource={{ kind: 'Unknown' }} fallback={resourceCategoryIcon('全部')} size={14} /></span><span class="catalog-label">全部资源</span><span>{resources.length}</span>
    </button>
    {#each Object.entries(resourceCategoryOptions).filter(([name]) => name !== '全部') as [categoryName, subtypes]}
      <div class="catalog-category">
        <button class:active={category === categoryName && subtype === '全部'} class="catalog-category-button" type="button" on:click={() => select(categoryName)}>
          <span class="catalog-name"><span class="catalog-icon"><ResourceBrandIcon resource={{ kind: categoryName }} fallback={resourceCategoryIcon(categoryName)} size={14} /></span>{categoryName}</span>
          ...
          <div class="catalog-subtypes">
            {#each subtypes as subtypeName}
              <button class:active={category === categoryName && subtype === subtypeName} type="button" on:click={() => select(categoryName, subtypeName)}>
                <span class="catalog-name"><span class="catalog-icon subtype-icon"><ResourceBrandIcon resource={{ kind: categoryName, subtype: subtypeName }} fallback={resourceCategoryIcon(subtypeName)} size={12} /></span>{subtypeName}</span>
