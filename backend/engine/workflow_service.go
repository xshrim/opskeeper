...
// existing Engine and Tool Gateway contracts. Node configuration is kept
...
func (s *WorkflowService) Execute(ctx context.Context, workflow Workflow, runID string) (WorkflowRun, error) {
	if s == nil || s.Engine == nil {
		return WorkflowRun{}, fmt.Errorf("workflow Engine is unavailable")
	}
...
func (s *WorkflowService) model(ctx context.Context, run WorkflowRun, node WorkflowNode) (WorkflowNodeResult, error) {
	if s.Engine == nil {
		return WorkflowNodeResult{}, fmt.Errorf("workflow Engine is unavailable")
	}
...
func (s *WorkflowService) tool(ctx context.Context, run WorkflowRun, node WorkflowNode) (WorkflowNodeResult, error) {
	...
	result, err := s.Gateway.Invoke(ctx, ToolCall{ExecutionID: run.ExecutionID, Sequence: run.Attempt, ScopeID: run.ScopeID, ResourceID: config.ResourceID, Name: config.Name, ProviderID: config.ProviderID, ModelName: config.ModelName, Arguments: arguments})
	if err != nil {
		return WorkflowNodeResult{}, err
	}
