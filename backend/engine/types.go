// Package engine provides the shared execution contract for AI-powered
...
var (
	ErrRunnerUnavailable = errors.New("Engine runner is unavailable")
	ErrInvalidRequest    = errors.New("Engine request is invalid")
	ErrAlreadyRunning    = errors.New("Engine execution is already running")
)
...
// optional execution plan. Engine owns invocation, policy and audit; plan
...
// it. This keeps the Agent Loop and all model/tool policy in Engine.
...
// Persona is a versioned expert prompt contract. It
...
// PersonaResolver resolves and authorizes a persona for an execution
// scope. Implementations should return only active, permitted profiles.
type PersonaResolver interface {
	Resolve(context.Context, string, string) (Persona, error)
}
...
func (r *Request) Normalize() error {
	...
	r.PersonaID = strings.TrimSpace(r.PersonaID)
...
// ModelBuilder resolves an Provider and model for one execution. Keeping
