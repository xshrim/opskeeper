...
>
  <header class="resource-add-main-heading">
    <div>
      <p class="eyebrow">
        ...
        editingDocker ||
        editingKubernetes ||
        editingHost || editingPostgreSQL || editingRedis
          ? 'EDIT RESOURCE'
          : 'ADD RESOURCE'}
      </p>
  ...
  <aside class="resource-add-steps" aria-label="添加资源步骤">
    ...
    {:else if kind === 'MCPServer'}
      ...
      <button
        class:active={step === 3}
        disabled={!mcpConfigurationComplete}
        type="button"
        on:click={() => mcpConfigurationComplete && onSelectStep(3)}
    ...
    {:else if kind === 'Docker'}
      <button
        ...
        type="button"
        on:click={() => basicConfigurationComplete && onSelectStep(2)}
        ><b>2</b><span>Docker 配置</span></button
      >
      <button
        class:active={step === 3}
        disabled={!dockerConfigurationComplete}
        type="button"
        on:click={() => dockerConfigurationComplete && onSelectStep(3)}
        ><b>3</b><span>总结核验</span></button
      >
    {:else if kind === 'Kubernetes'}
      <button
        ...
        type="button"
        on:click={() => basicConfigurationComplete && onSelectStep(2)}
        ><b>2</b><span>Kubernetes 配置</span></button
      >
      <button
        class:active={step === 3}
        disabled={!kubernetesConfigurationComplete}
        type="button"
        on:click={() => kubernetesConfigurationComplete && onSelectStep(3)}
        ><b>3</b><span>总结核验</span></button
      >
    {:else if kind === 'Host'}
      <button
        ...
        type="button"
        on:click={() => basicConfigurationComplete && onSelectStep(2)}
        ><b>2</b><span>Host 配置</span></button
      >
      <button
        class:active={step === 3}
      ...
      <div class="resource-add-step-actions">
        ...
        {:else if kind === 'MCPServer' && step === 2}
          <button
            ...
            type="button"
            on:click={() => onSelectStep(1)}>上一步</button
          ><button class="primary" type="button" on:click={onContinueMcp}
            >下一步</button
          >
        {:else if kind === 'MCPServer' && step === 3}
          ...
          ><button
            class="primary"
            type="button"
            on:click={onSubmitMcp}
            disabled={busy || !scopeSelected}
            >{editingResource ? '保存' : '创建'}</button
          >
        {:else if kind === 'Docker' && step === 2}
          <button
            ...
            type="button"
            on:click={() => onSelectStep(1)}>上一步</button
          ><button class="primary" type="button" on:click={onContinueDocker}
            >下一步</button
          >
        {:else if kind === 'Docker' && step === 3}
          ...
          ><button
            class="primary"
            type="button"
            on:click={onSubmitDocker}
            disabled={busy || !scopeSelected}
            >{editingDocker ? '保存' : '创建'}</button
          >
        {:else if kind === 'Kubernetes' && step === 2}
          ...
          ><button
            class="primary"
            type="button"
            on:click|preventDefault={() => onContinueKubernetes()}
            >下一步</button
          >
        {:else if kind === 'Kubernetes' && step === 3}
          ...
          ><button
            ...
            on:click={onSubmitDocker}
            disabled={busy || !scopeSelected}
            >{editingKubernetes ? '保存' : '创建'}</button
          >
        {:else if kind === 'Host' && step === 2}
          <button
            class="secondary"
            type="button"
            on:click={() => onSelectStep(1)}>上一步</button
          ><button class="primary" type="button" on:click={onContinueHost}
            >下一步</button
          >
        {:else if kind === 'Host' && step === 3}
          <button
            class="secondary"
            type="button"
            on:click={() => onSelectStep(2)}>上一步</button
        ...
        {:else if kind === 'PostgreSQL' && step === 2}
        ...
        {:else if kind === 'Redis' && step === 3}
          <button class="secondary" type="button" on:click={() => onSelectStep(2)}>上一步</button><button class="primary" type="button" on:click={onSubmitRedis} disabled={busy || !scopeSelected}>{editingRedis ? '保存' : '创建'}</button>
        ...
        {:else if step === 2}
          ...
          ><button
            ...
            disabled={busy || !scopeSelected}>创建</button
          >
        {/if}
      </div>
    </div>
