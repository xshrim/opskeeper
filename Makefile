...
.PHONY: help deps dev-services-up dev-services-down dev-services-logs run-api run-worker run-scheduler run-frontend test backend-test frontend-test lint backend-lint frontend-lint format format-check build quality
...
run-api: ## Run the API server.
	set -a; source $(ENV_FILE); set +a; cd backend && go run ./cmd/api

run-worker: ## Run the worker process.
	set -a; source $(ENV_FILE); set +a; cd backend && go run ./cmd/worker

run-scheduler: ## Run the scheduler process.
	set -a; source $(ENV_FILE); set +a; cd backend && go run ./cmd/scheduler
...
lint: backend-lint frontend-lint ## Run backend and frontend static checks.
