<script lang="ts">
  ...
  $: connectionStatus = resourceCheck
    ? resourceCheck.status === 'succeeded' ? `正常 · ${resourceCheck.latency_ms} ms` : '连接异常'
    : accessMode === 'agent' ? '由 MCPServer 负责' : '尚未测试';
</script>
...
<div class="provider-resource-details docker-resource-details">
  <div class="provider-resource-meta">
    ...
    <div class="provider-resource-connection"><span>连接状态</span><strong>{connectionStatus}</strong>{#if resourceCheck?.status === 'failed'}<small>{resourceCheck.message}</small>{/if}</div>
