<script lang="ts">
  ...
  async function saveEdit(app: Application) { try { await api.updateApplication(selectedProjectId, app.id, { name: editName, description: editDescription }); editingAppId = ''; await loadWorkspace(selectedProjectId); onNotice('应用已更新'); } catch (error) { onError(describeError(error, '更新应用失败')); } }
