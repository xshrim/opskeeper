...
- AIEngine（内部包含 LLMEndpoint）、MCP Server、Skill 等 AI 运维能力。
...
| Connector | Kubernetes、Prometheus、Loki 能力适配和连接检查 | 已实现并验收，T09；中间件和 AIEngine 在后续目标设计中扩展 |
...
| `/opskeeper/api/v1/ai-engines`、`/opskeeper/api/v1/ai-engines/{id}/test` | 目标设计；替代历史 LLM Provider 接口 |
