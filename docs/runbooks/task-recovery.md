# Worker、Scheduler 与缓存故障恢复

1. 检查 PostgreSQL readiness、配置的缓存后端状态、Worker/Scheduler Pod 重启和 `opskeeper.errors` 指标。
2. 缓存后端不可用时，授权缓存会回源 PostgreSQL；不要为恢复缓存而放宽授权。后端恢复后允许缓存自然重建。
