...
{:else}
<div class="docker-form-grid"><label class:invalid={configurationAttempted && !brokers.trim()}><span><i>*</i>Broker 地址</span><textarea bind:value={brokers} rows="3" placeholder="每行一个 broker，例如 kafka-1:9092" on:input={onConfigurationChange}></textarea></label><label><span>启用 TLS</span><input type="checkbox" bind:checked={tls} on:change={onConfigurationChange} /></label><label><span>TLS Server Name</span><input bind:value={tlsServerName} on:input={onConfigurationChange} /></label><label><span>用户名</span><input bind:value={username} on:input={onConfigurationChange} /></label><label><span>密码</span><PasswordInput bind:value={password} on:input={onConfigurationChange} ariaLabel="密码" /></label><label><span>超时时间（秒）</span><input type="number" min="1" max="300" bind:value={timeoutSeconds} on:input={onConfigurationChange} /></label></div>
{/if}
