...
var (
	ErrToolNotFound = errors.New("Engine tool was not found")
	ErrToolDenied   = errors.New("Engine tool call was denied")
	ErrToolInvalid  = errors.New("Engine tool request is invalid")
	ErrToolLimited  = errors.New("Engine tool response exceeds the configured limit")
)
...
func (g *PolicyGateway) Invoke(ctx context.Context, call ToolCall) (ToolResult, error) {
	...
	if call.ResourceID == "" || call.Name == "" {
		...
		g.recordToolCall(ToolCallRecord{ExecutionID: call.ExecutionID, Sequence: call.Sequence, ProviderID: call.ProviderID, ModelName: call.ModelName, ResourceID: call.ResourceID, ToolName: call.Name, Arguments: call.Arguments, Status: StatusFailed, ErrorCode: "tool_validation", Error: message, StartedAt: at, CompletedAt: at})
	...
	if !ok {
		...
		message := publicError(err)
		at := time.Now().UTC()
		g.recordToolCall(ToolCallRecord{ExecutionID: call.ExecutionID, Sequence: call.Sequence, ProviderID: call.ProviderID, ModelName: call.ModelName, ResourceID: call.ResourceID, ToolName: call.Name, Arguments: call.Arguments, Status: StatusFailed, ErrorCode: "tool_not_found", Error: message, StartedAt: at, CompletedAt: at})
	...
	if g.Authorize != nil {
		if err := g.Authorize(ctx, call, definition); err != nil {
			...
			g.recordToolCall(ToolCallRecord{ExecutionID: call.ExecutionID, Sequence: call.Sequence, ProviderID: call.ProviderID, ModelName: call.ModelName, ResourceID: call.ResourceID, ToolName: definition.Name, Arguments: call.Arguments, Status: StatusFailed, ErrorCode: "tool_denied", Error: message, StartedAt: at, CompletedAt: at})
	...
	case <-ctx.Done():
		...
		at := time.Now().UTC()
		g.recordToolCall(ToolCallRecord{ExecutionID: call.ExecutionID, Sequence: call.Sequence, ProviderID: call.ProviderID, ModelName: call.ModelName, ResourceID: call.ResourceID, ToolName: definition.Name, Arguments: call.Arguments, Status: StatusFailed, ErrorCode: "cancelled", Error: message, StartedAt: at, CompletedAt: at})
		_ = emitToolEvent(call.EventSink, Event{Type: "tool.failed", Status: StatusFailed, Payload: toolEventPayload(call, definition.Name, map[string]any{
			"error":       message,
	...
	if err != nil {
		message := publicError(err)
		g.recordToolCall(ToolCallRecord{ExecutionID: call.ExecutionID, Sequence: call.Sequence, ProviderID: call.ProviderID, ModelName: call.ModelName, ResourceID: call.ResourceID, ToolName: definition.Name, Arguments: call.Arguments, Status: StatusFailed, ErrorCode: "tool_execution", Error: message, StartedAt: started, CompletedAt: completedAt, DurationMS: durationMS})
